...
app = FastAPI(title="Whisper Subtitles")
...
def index(request: Request):
    ...
    return templates.TemplateResponse("index.html", {"request": request, "max_upload_mb": max_upload_mb})
...
async def upload(request: Request, background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    background_tasks.add_task(process_job, job_id, str(dest_path))
...
async def translate_view(request: Request, job_id: str, target_lang: str = Form(...)):
    ...
    src_lang = job["source_lang"] or "auto"
